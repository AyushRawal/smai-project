...
train_loader = DataLoader(
    HeightDataset(X_train, y_train), batch_size=batch_size, shuffle=True
)
val_loader = DataLoader(
    HeightDataset(X_val, y_val), batch_size=batch_size, shuffle=False
)
...
for epoch in range(1, epochs + 1):
    ...
    if epoch == epochs or (
        epochs_no_improve + 1 >= patience and val_acc <= best_val_acc
    ):
